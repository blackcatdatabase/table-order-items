...
final class OrderItemRepository implements RepoContract, KeysetRepoContract
{
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\OrderItems\Definitions::table());
    ...
    */
    ...
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\OrderItems\Dto\OrderItemDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\OrderItems\Mapper\OrderItemDtoMapper::fromRow($row);
    }
