...
namespace BlackCat\Database\Packages\OrderItems\Repository;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\OrderItems\Definitions;
use BlackCat\Database\Packages\OrderItems\Criteria;
use BlackCat\Database\Packages\OrderItems\ContractRepository;

final class OrderItemRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO order_items (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO order_items (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO order_items (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE order_items SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE order_items SET $set WHERE id = :id", $params);
        }
        return $this->db->execute("DELETE FROM order_items WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE order_items SET $set WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM order_items WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM order_items WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM order_items t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM order_items t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM order_items WHERE id = :id FOR UPDATE"
            : "SELECT * FROM order_items WHERE id = :id FOR UPDATE";
