...
namespace BlackCat\Database\Packages\OrderItems\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\OrderItems\Dto\OrderItemDto;
use BlackCat\Database\Packages\OrderItems\Mapper\OrderItemDtoMapper;
 ...
 */
final class OrderItemsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private OrderItemRepository $orderItemRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'order_items' => $this->orderItemRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\OrderItems\ModuleException("Record {$idStr} not found for locking.");
            }
