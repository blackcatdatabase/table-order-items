...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 3 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 2 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\order-items](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\order-items)
...
graph LR
  ...
  order_items["order_items"]:::center
  order_items -->|FK| books["books"]:::link
  order_items -->|FK| orders["orders"]:::accent
  order_items -->|FK| tenants["tenants"]:::link
...
- Outbound (depends on): "books", "orders", "tenants"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package order-items -Apply
